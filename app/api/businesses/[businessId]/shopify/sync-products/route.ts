...
export async function POST(req: Request, context: RouteContext) {
  try {
    ...
    runShopifySync(job.id).catch((error) => {
      console.error('Shopify sync runner failed', error)
    })
...
async function runShopifySync(jobId: string) {
  try {
    const { data: job, error: jobError } = await supabaseAdmin
      .from('shopify_sync_jobs')
      .select('id, business_id, warnings')
      .eq('id', jobId)
      .single()

    if (jobError || !job) {
      throw new Error(jobError?.message || 'Shopify sync job not found')
    }
    ...
    if (!shopDomain || !accessToken) {
      throw new Error('Connect Shopify before syncing products')
    }

    if (!isAllowedShopifyDomain(shopDomain)) {
      throw new Error('Use a valid myshopify.com shop domain')
    }
